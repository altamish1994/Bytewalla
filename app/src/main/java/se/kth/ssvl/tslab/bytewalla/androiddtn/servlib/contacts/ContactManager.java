...
import se.kth.ssvl.tslab.bytewalla.androiddtn.systemlib.storage.thread.VirtualTimerTask;
...
public class ContactManager extends BundleEventHandler {
	...
	public boolean add_new_link(Link link) {
		...
		try {
			...
			links_.add(link);
			Log.i(TAG, "ADDING NEW LINK ID " + link.remote_eid().toString());
