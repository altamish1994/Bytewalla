			...
			if(bundle.custodian()!=null&& !bundle.custodian().equals(EndpointID.NULL_EID()))
			{
				...
				//if(!custodian.equals(ID)) {
					...
					notify_msg += "Guest ";
						...
						if( MultiHopStorage.getInstance().impt_sqlite_().get_records("Forwards", "bundle_id = "+bundle.bundleid()+
								 " and destination = '" + bundle.custodian().toString()+"'", "id").size() > 0 )
						{
							return ;
						}
					    // check if this bundle already exists in my database, discard it
                        if( MultiHopStorage.getInstance().impt_sqlite_().get_records("bundle", "bundle_id = "+bundle.bundleid()
                              +" and source = '"+bundle.source()+"'"
...
//			else Toast.makeText(DTNManager.getInstance().getApplicationContext(),"From " + bundle.source().toString(),Toast.LENGTH_LONG).show();
		Log.d("RECIVINGS","forwarded From " + bundle.source().toString());
				DTNManager.getInstance().notify_user(notify_msg+"DTN Bundle Received", "From " + bundle.source().toString());
...
//						}
						...
						if( MultiHopStorage.getInstance().impt_sqlite_().get_records("Forwards", "bundle_id = "+bundle_id+
								" and destination = '" + link+"'", "id").size() ==0 )
						{
